...
Всегда будь дружелюбным и подтверждай выполнение действий.';
    ...
    public static function InitUserData(string $name, string $about){
        ...
        // Создаем базовый промпт с информацией о пользователе
        $userInfo = "Пользователя зовут '$name' поэтому всегда обращайся к нему по имени. Вот информация о пользователе: $about\n\n";
        
        // Инициализируем базовый системный промпт
        self::initializeSystemPrompt();
        
        // Добавляем информацию о пользователе в начало промпта после приветствия
