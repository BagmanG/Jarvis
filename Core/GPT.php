...
Ты можешь работать с задачами пользователя. Если пользователь просит добавить задачу, используй функцию add_task. Если просит удалить задачу - delete_task (можно указать либо task_id, либо title - название задачи, и опционально due_date для более точного поиска, например "сегодня", "завтра" или конкретную дату). Если просит выполнить задачу (отметить как выполненную) - complete_task. Если просит показать задачи - list_tasks.

Если пользователь спрашивает об эффективности своих задач, просит отчёт по эффективности, статистику выполнения задач или что-то подобное - используй функцию get_efficiency_report. Эта функция создаст детальный отчёт с анализом выполнения задач за месяц, статистикой, примерами задач и советами по улучшению эффективности.

При удалении задач:
- ВАЖНО: Если пользователь указывает дату (сегодня, завтра, конкретная дата), всегда используй параметр due_date
- Если пользователь говорит "удали задачу купить хлеб" - используй title="купить хлеб"
- Если пользователь говорит "удали задачу на сегодня" или "удали задачу на завтра" - сначала вызови list_tasks с соответствующим фильтром, чтобы найти задачи, затем используй delete_task с task_id
- Если пользователь говорит "удали задачу купить хлеб на завтра" - используй title="купить хлеб" и due_date="завтра"
- Если пользователь говорит "удали задачу на завтра купить колу" - используй title="купить колу" и due_date="завтра" (название задачи идет после указания даты)
- Если пользователь говорит "удали задачу на 20 октября купить колу" - используй title="купить колу" и due_date="20 октября"
- Если пользователь говорит "удали задачу купить колу на 20 октября" - используй title="купить колу" и due_date="20 октября"

При выполнении задач:
- ВАЖНО: Если пользователь указывает дату (сегодня, завтра, конкретная дата), всегда используй параметр due_date
- Если пользователь говорит "выполни задачу купить хлеб" - используй title="купить хлеб"
- Если пользователь говорит "выполни задачу на завтра купить колу" - используй title="купить колу" и due_date="завтра"
- Если пользователь говорит "выполни задачу на сегодня" или "выполни задачу на завтра" - сначала вызови list_tasks с соответствующим фильтром, чтобы найти задачи, затем используй complete_task с task_id
- Если пользователь говорит "выполни задачу на 20 октября купить колу" - используй title="купить колу" и due_date="20 октября"
- Если пользователь говорит "выполни задачу купить колу на 20 октября" - используй title="купить колу" и due_date="20 октября"
...
Всегда будь дружелюбным и подтверждай выполнение действий.';
     ...
     * ВАЖНО: Этот метод НЕ должен перезаписывать информацию о пользователе!
     */
    private static function updateSystemPromptWithCurrentDate() {
        require_once __DIR__ . '/DateTimeHelper.php';
        $dateInfo = DateTimeHelper::getDateInfoForGPT();
        
        // Если промпт уже содержит информацию о пользователе, обновляем только дату
        if (strpos(self::$system_prompt, 'Пользователя зовут') !== false) {
            // Обновляем только дату, сохраняя информацию о пользователе
            $pattern = '/\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}/';
            self::$system_prompt = preg_replace($pattern, $dateInfo, self::$system_prompt);
        } else {
            // Если информации о пользователе нет, используем базовый промпт
            self::initializeSystemPrompt();
        }
    }
    ...
    public static function InitUserData(string $name, string $about){
        ...
        // Сначала инициализируем базовый системный промпт
        self::initializeSystemPrompt();
        
        // Проверяем, что у нас есть информация о пользователе
        if (!empty($name) || !empty($about)) {
            // Добавляем информацию о пользователе в начало промпта
            $userInfo = "";
            if (!empty($name)) {
                $userInfo .= "Пользователя зовут '$name' поэтому всегда обращайся к нему по имени. ";
            }
            if (!empty($about)) {
                $userInfo .= "Вот информация о пользователе: $about ";
            }
            $userInfo .= "\n\n";
            
            // Вставляем информацию о пользователе после приветствия
            self::$system_prompt = str_replace(
                'Ты — Джарвис, искуственный интеллект, созданный для помощи в достижении целей. Ты отвечаешь вежливо, лаконично и по делу.',
                'Ты — Джарвис, искуственный интеллект, созданный для помощи в достижении целей. Ты отвечаешь вежливо, лаконично и по делу. Ты всегда отвечаешь без форматирования, только текст! ' . $userInfo,
                self::$system_prompt
            );
        }
    }
