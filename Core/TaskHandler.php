...
class TaskHandler {
    ...
    // Получение отчёта об эффективности за месяц
    public static function getEfficiencyReport($userId): array {
        try {
            $mysqli = self::getConnection();
            
            // Получаем задачи за последний месяц
            ...
            if (empty($tasks)) {
                ...
                return [
                    'success' => true,
                    'message' => 'У вас нет задач за последний месяц для анализа',
                    'stats' => []
                ];
            }
            ...
            foreach ($tasks as $task) {
                if ($task['status'] === 'completed') {
                    $completedTasks++;
                    if (count($completedExamples) < 3) {
                        $completedExamples[] = $task;
                    }
                } else {
                    $pendingTasks++;
                    if (count($pendingExamples) < 3) {
                        $pendingExamples[] = $task;
                    }
            ...
            $report = [
                ...
                'pending_examples' => $pendingExamples,
                'all_tasks' => $tasks
            ];
