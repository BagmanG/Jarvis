...
export function initProfileView({ onOpen, onClose, onSave, onSaveTheme }) {
  ...
  store.subscribe((state) => {
    ...
    const telegramLine = document.getElementById('profileTelegramLine');
    const themeModeControl = document.getElementById('themeModeControl');
    const weekStartControl = document.getElementById('weekStartControl');
    const accentPaletteEl = document.getElementById('accentPalette');

    if (displayNameInput) displayNameInput.value = profile.display_name || '';
    if (telegramLine) telegramLine.textContent = profile.telegram_username ? `@${profile.telegram_username}` : 'username недоступен';
    renderAvatar(profile);
    if (themeModeControl) themeModeControl.dataset.selectedValue = settings.theme_mode;
    if (weekStartControl) weekStartControl.dataset.selectedValue = String(settings.week_start);
    if (accentPaletteEl) accentPaletteEl.dataset.selectedAccent = settings.accent_color;
    syncPaletteSelection();
